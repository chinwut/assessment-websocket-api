...
describe('messageHandler', () => {
  ...
  it('should handle broadcast message type from admin user', () => {
    const message = JSON.stringify({ type: 'broadcast', message: 'Hello everyone' });
  ...
  it('should handle direct message type', () => {
    const message = JSON.stringify({ type: 'direct', to: 'user2', from: 'user1', message: 'Hello user2' });
    fakeClients.set(mockWs, { isAdmin: false }); 

    messageHandler(mockWs, message);

    expect(sendDirectMessage).toHaveBeenCalledWith('user2', 'Hello user2', 'user1');
});
